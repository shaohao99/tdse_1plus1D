...
// fft for complex 1D
void fft_complex_1D (const int &n, vector<complex> &input_f, vector<complex> &output_f)
{
  fftw_complex *in;
  fftw_complex *out;
...
void ft_complex_1D (vector<complex> &input, vector<double> &input_x, const int n_inp, const double inpstep, vector<complex> &output, vector<double> &out_x, const double outmin, const double outmax, const double outstep, const dump_type dump, const ft_type type, const double damp_f)
{
...
// calculate Gamma fucntion by Stirling approximation, Windschitl's formula: http://en.wikipedia.org/wiki/Stirling%27s_approximation
  complex Gamma_Stirling_Windschitl (complex z)
  {
    
    complex value = z/exp(1.) * sqrt( z * Sinh(1./z) + 1./(810.*pow(z,6.)) );
    complex Gamma_z = sqrt(2.*pi/z) * pow(value, z);
    return Gamma_z;

  }

// calculate Gamma fucntion by Stirling approximation, Nemes' formula: http://en.wikipedia.org/wiki/Stirling%27s_approximation
  complex Gamma_Stirling_Nemes (complex z)
  {

    complex value = 1./exp(1.) * ( z + 1./( 12.*z - 1./(10.*z) ) );
    complex Gamma_z = sqrt(2.*pi/z) * pow(value, z);
    return Gamma_z;

  }

// calculate Gamma fucntion by Lanczos approximation: http://en.wikipedia.org/wiki/Lanczos_approximation
  inline complex Gamma_Lanczos (complex z)
  {
    ...
    else
    {
        z -= 1;
        x = p[0];
        for( int i=1; i<=9; i++) x += p[i-1]/(z+double(i));
        t = z + g + 0.5;
