...
Parameter<vector<int> > n_points( "n_points", vector<int>(2,2000), "number of points for (X1,X2,X3)" );
Parameter<vector<int> > n_points_g( "n_points_g", vector<int>(2,200), "number of points for (X1,X2,X3)" );
Parameter<vector<double> > spatial_step( "spatial_step", vector<double>(2,0.3), "spatial_steps for (X1,X2,X3)" );

Parameter<double> dt_i("dt_i",0.05, "Time step for imaginary time propagation");
Parameter<double> dt_p("dt_p",0.05, "Time step for pulse 1");
...
Parameter<double> lambda1("lambda1",800., "Wavelength of the Laser (nm)"); 
Parameter<double> i1("i1",1.0e12, "Intensity of the Laser (W/cm2)");
Parameter<double> cep1("cep1",0., "fraction of Pi for the initial phase");
Parameter<double> n_c1("n_c1",6., "Number of cycles of the as pulse");
Parameter<double> i2("i2",1.0e8, "Intensity of the Laser (W/cm2)");
Parameter<double> n_c2("n_c2",8., "Number of cycles of the as pulse");
Parameter<double> cep2("cep2",0., "fraction of Pi for the initial phase");
Parameter<int> nenv("nenv",2, "Sin^n envelope");
Parameter<int> nhhg("nhhg",21, "Number of HHG");
Parameter<double> ohhg0("ohhg0",61.75, "Starting number of HHG");
Parameter<double> delay("delay",0., "Delay between pulses");  // unit: cycle
Parameter<double> t_wait("t_wait",2., "Time for free propogation");

Parameter<double> x_SI("x_SI",4., "Single ionization regoin.");
Parameter<double> fact("fact",6., "Multiple of time for FFT.");
Parameter<double> facout("facout",0.04, "Fraction of time for FFT output.");
Parameter<int> fft_sign("fft_sign",-1, "forward or backward fft");

Parameter<int> facinter("facinter",250, "How many time steps absorber is appled?");
Parameter<double> epsilon("epsilon",1.e-15, "Absorbing potential.");
Parameter<double> v0("v0",2., "Absorbing potential.");
Parameter<double> tf("tf",0., "Final time for Volkov propagation."); // a.u.
Parameter<double> facp("facp",120., "Multiple of momentum for FFT.");
Parameter<double> Emin("Emin",0., "Min of energy spectrum");
Parameter<double> Emax("Emax",100., "Max of energy spectrum");
Parameter<double> dE("dE",0.02, "Max of energy spectrum");

Parameter<double> a_percent1("absorb_percent1",0.1,"on how many percents of the grids are the wf absorbed?");
Parameter<double> a_percent2("absorb_percent2",0.1,"on how many percents of the grids are the wf absorbed?");
Parameter<double> mask_exp("mask_exponent",0.167,"the exponent for cos function");

Parameter<string> jobname("jobname", "results", "The Name of the job");
Parameter<string> outputFolder("outputFolder", "results", "The basename of the output directory");  // just a default, is set in Input.cpp from jobname()
